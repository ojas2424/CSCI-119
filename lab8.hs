...
compl (q, s1, f, d1) = (qs, s, fs, d) where
    qs  = q
    s   = s1
...
quot_right ws (qs1, s1, fs1, d1) = (qs, s, fs, d) where
	qs  = qs1
	s   = s1
	fs  = [q | q <- qs1, or [elem (star d q w) fs1 | w <- ws]] 
	d q a = d1 q a
...
nap_hat d qs a = norm $ concat[d q' a | q <- x, q' <- q] where
    x = power qs
...
eacc (qs1, s1, fs1, d1, es1) ws =  overlap (eap_hat_star (d1, es1) (eclose es1 s1) ws) fs1 where
...
string_ab :: EFSM Int
string_ab = (qs, ss, fs, d, es) where
    qs  = [0, 1, 2, 3, 4, 5]
    ss  = [0]
    fs  = [4]
    d 0 'a' = [1]
    d 3 'b' = [4]
    d q  _  = if q <= 2 then [2] else [5]
    es = [(1,3)]
