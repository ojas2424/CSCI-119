...
solve ((l11:l1J) : rows) (l1':lI') = simp x1 : xI where
  ...
  seven li1 li' = Union' [ Cat' [li1, Star' l11, li'], li']
  ...
  x1 =
    Cat' [Star' l11, 
    Union' (zipWith (\lij xi -> Cat' [lij, xi]) l1J xI ++ [l1'])]
...
toSPLE (qs,s,fs,d) = (lIJ, lI') where
  ...
  coef i j = Union' [Let' a | a <- sigma, qi <- qs, qj <- qs, (d qi a) == qj]

  -- Construct constants
  lI' = [ check qi | qi <- qs] where
    check q = if elem q fs then One else Zero
